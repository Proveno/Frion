...
const AdminHealingSearchList = ({ Akey, isKeyValid, keyData, requests, searchText, locale }) => {
  ...
  const [deletingHealingId, setDeletingHealingId] = useState();
  const [acceptingHealingId, setAcceptingHealingId] = useState();
  useEffect(async () => {
    if (deletingHealingId) {
      const deleted = await fetch(
        `process.env.API_HOSTing/request/${deletingHealingId}`,
        {
          method: "DELETE"
        }
      );
  ...
  useEffect(async () => {
    if (acceptingHealingId) {
      const accepted = await fetch(
        `process.env.API_HOSTing/request/${acceptingHealingId}`,
        {
          method: "PUT",
          headers: {
                              ...
                              <div className="absolute bottom-0 right-0 w-full px-4 pb-4">
                                <div className="w-full py-1 flex">
                                      <button
                                        ...
                                        onClick={() => {
                                          setAcceptingHealingId(request._id);
                                        }}
                                      >
                                        {t.acceptBtn}
                                      </button>
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        setDeletingHealingId(request._id);
                                      }}
                                    >
                                      {t.deleteBtn}
                                    </button>
...
export default AdminHealingSearchList;