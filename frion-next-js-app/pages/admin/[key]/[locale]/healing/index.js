...
const AdminHealingList = ({ Akey, isKeyValid, keyData, requests, locale }) => {
  ...
  useEffect(async () => {
    if (deletingHealingId) {
      const deleted = await fetch(
        `http://localhost:3000/api/healing/request/${deletingHealingId}`,
        {
          method: "DELETE"
        }
      );
...
AdminHealingList.getInitialProps = async ({ query: { key, locale } }) => {
  ...
  const res = await fetch(`http://localhost:3000/api/healing/${locale}`);
...
export default AdminHealingList;
